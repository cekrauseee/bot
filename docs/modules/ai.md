...
The request's `working_directory` defaults to `/workspace` and must be a canonical absolute path within that workspace. Root and child agents receive the shared root and current directory as factual context. Runtime tool envelopes carry this directory; relative file paths and omitted filesystem-list/shell paths resolve there in the runtime. Absolute paths elsewhere in the same workspace remain accessible. This adds no project-specific behavior policy or filesystem interface.
