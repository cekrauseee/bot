...
`POST /agent/stream` accepts only `Authorization: Bearer <AI_SERVICE_TOKEN>`. Its strict v2 request includes user, workspace, run, conversation, and turn identifiers; the frozen working directory; ordered text messages; task-plan context; and model settings. The response envelope repeats the run and turn identifiers and emits ordered provider-neutral events for checkpoints, reasoning, text, web search, plan updates, runtime tools, child agents, browser frames, completion, and failure.
