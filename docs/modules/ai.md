...
`POST /agent/stream` accepts only `Authorization: Bearer <AI_SERVICE_TOKEN>`. Its strict v2 request includes user, workspace, run, conversation, and turn identifiers; the frozen working directory; ordered text messages; task-plan context; and model settings. The response envelope repeats the run and turn identifiers and emits ordered provider-neutral events for checkpoints, reasoning, text, web search, plan updates, runtime tools, child agents, browser frames, completion, and failure. Content blocks retain provider order. Reasoning summaries remain additive deltas and receive a stable `activity_id` for each contiguous visible phase, so a tool or task boundary creates a new chronological activity. A delegated child exposes its own `child.started` and `child.completed` lifecycle; descendant model, tool, and browser events do not become root response activity. Hosted MCP calls expose `tool.started` and later updates only when the provider reports those observations. A terminal first observation emits only `tool.completed`, without a synthetic start or tool output.
