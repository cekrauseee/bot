...
`POST /tools` requires the runtime bearer token and a strict v2 request containing user, workspace, conversation, run, operation, tool, and argument fields. The current model-visible tools are:
