...
The current `apps/web-rebuild` account dropdown also owns the OpenAI Codex provider connection. It exposes one Connect action and uses the server-selected, validated login mode: browser mode opens the returned `auth_url`, while device mode shows the returned `verification_url` and `user_code`. Both modes poll the same returned attempt ID, support cancellation, and show connected-account limits plus login limits as plain secondary information. The browser never receives or stores ChatGPT access or refresh tokens.
