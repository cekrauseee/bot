...
Setup aligns the root `.env` with `.env.example`, generates local secrets without printing them, installs workspace dependencies, builds email assets, starts infrastructure, and applies migrations. The root environment is shared by Vite, the API, AI, and runtime. `VITE_API_BASE_URL` is the only browser-exposed value. Google and Resend credentials are optional for local UI work.
...
| `npm run desktop:package` | Build an unsigned local desktop package |
