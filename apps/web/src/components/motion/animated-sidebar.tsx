...
import {
  ...
  useAnimatedSidebar,
} from "@/components/motion/animated-sidebar-context";
import {
  EASE_DRAWER,
  EASE_OUT,
  SPRING_LAYOUT,
...
export function AnimatedSidebarProvider({
  ...
  const [trigger, registerTrigger] = useState<HTMLButtonElement | null>(null);
...
function MobileSidebar({
  ...
  }, [context.openMobile, context.trigger]);
...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <div
      ...
      className={cn(
        "mb-1 h-7 overflow-hidden px-2 text-xs font-medium text-muted-foreground transition-opacity",
...
export function AnimatedSidebarMenuButton({
  ...
  const content = (
    <>
      ...
      {icon ? (
        <span
          aria-hidden="true"
          className="relative z-10 grid size-5 shrink-0 place-items-center [&_svg]:size-4 [&_svg]:shrink-0"
        >
          {icon}
        </span>
      ) : null}
  ...
  const interactiveClassName = cn(
    "relative flex min-h-9 w-full min-w-0 items-center gap-2.5 overflow-hidden rounded-xl px-3 text-left text-xs font-medium leading-4 outline-none",
