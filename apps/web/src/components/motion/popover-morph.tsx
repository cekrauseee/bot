...
export function MorphPopover({
  ...
  const close = useCallback(() => {
    ...
    const inPanel =
      focused instanceof HTMLElement && content?.contains(focused);
    if (!inPanel) return;
    const restore = trigger ?? (root && root.tabIndex >= 0 ? root : null);
    restore?.focus();
  }, [content, root, setOpen, trigger]);
  ...
  const ctx = useMemo<MorphContextValue>(
    ...
    [anchor, baseId, content, open, setOpen, toggle],
  );
...
export function MorphPopoverContent({
  ...
  const { contentId, registerContent, triggerId } = ctx;
