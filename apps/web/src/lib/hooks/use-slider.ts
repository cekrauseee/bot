...
export interface SliderOptions {
  ...
  step?: number;
  disabled?: boolean;
  /** Horizontal inset of the usable track, in px. */
  trackInset?: number;
  "aria-label"?: string;
...
export function useSlider({
  ...
  const commitFromX = useCallback(
    (clientX: number) => {
      ...
      const usableWidth = Math.max(0, rect.width - trackInset * 2);
      const ratio = usableWidth
        ? clamp((clientX - rect.left - trackInset) / usableWidth, 0, 1)
        : 0;
      commit(lo + ratio * (hi - lo));
    },
    [commit, lo, hi, trackInset],
  );
