...
function TaskHeaderIcon({ complete }: { complete: boolean }) {
  ...
  return (
    ...
    >
      <button
        ...
        className="group flex h-10 w-full items-center gap-2.5 rounded-2xl px-3.5 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        <TaskHeaderIcon complete={allComplete} />
        <h3 className="min-w-0 flex-1 truncate text-xs font-medium text-foreground/90">
          {title}
        </h3>
        <span
          className={cn(
      ...
      >
        ...
        >
          {items.length ? (
            <ol aria-live="polite" className="flex flex-col gap-0">
              <AnimatePresence initial={false} mode="popLayout">
                {items.map((item) => {
                  ...
                  return (
                    <motion.li
                      ...
                      className="flex min-h-8 items-center gap-2.5 rounded-xl px-1.5 py-1"
                    >
                      <TaskStatusIcon status={status} progress={item.progress} />
