...
test('desktop package consumes the canonical web artifact and uses a hardened renderer', async () => {
  ...
  const preload = await readFile(`${projectRoot}/apps/desktop/src/preload.cts`, 'utf8')
  ...
  assert.match(main, /DEEP_LINK_PREFIX = .*auth\/callback/)
  assert.match(main, /Authorization = `Bearer/)
  assert.doesNotMatch(main, /shell\.openExternal\(result\.client_secret\)/)
  assert.doesNotMatch(main, /setTimeout\(resolve, 1_000\)/)
  assert.match(main, /app\.setAsDefaultProtocolClient\(DEEP_LINK_SCHEME/)
  assert.match(main, /app\.requestSingleInstanceLock\(\)/)
  assert.match(main, /app\.on\('open-url'/)
  assert.match(main, /app\.on\('second-instance'/)
  assert.match(main, /safeStorage\.encryptString\(JSON\.stringify\(value\)\)/)
  assert.match(config, /protocols: \[\{ name: 'Bot authentication', schemes: \['mybot'\] \}\]/)
...
test('icon source and install workflow are deterministic and scoped', async () => {
  ...
  assert.match(installer, /Bot-darwin/)
...
test('desktop development prepares local assets before Electron starts', async () => {
  const rootPackage = JSON.parse(await readFile(`${projectRoot}/package.json`, 'utf8'))
  const desktopPackage = JSON.parse(await readFile(`${projectRoot}/apps/desktop/package.json`, 'utf8'))
  const developmentScript = await readFile(`${projectRoot}/apps/desktop/scripts/dev.mjs`, 'utf8')
  assert.match(rootPackage.scripts['desktop:dev'], /--filter=@my-bot\/web/)
  assert.match(rootPackage.scripts['desktop:dev'], /--filter=@my-bot\/desktop/)
  assert.match(desktopPackage.scripts.predev, /icons:build/)
  assert.match(desktopPackage.scripts.predev, /build-public-config\.mjs/)
  assert.doesNotMatch(desktopPackage.scripts.predev, /build --workspace=@my-bot\/web/)
  assert.equal(desktopPackage.scripts.dev, 'node scripts/dev.mjs')
  assert.match(developmentScript, /Wait for the Vite renderer/)
  assert.match(developmentScript, /watch\(sourceRoot/)
  assert.match(developmentScript, /spawn\(electronPath/)
  assert.match(desktopPackage.scripts.build, /clean-dist\.mjs/)
  await access(`${projectRoot}/apps/desktop/scripts/clean-dist.mjs`)
})

test('desktop uses the Vite renderer in development and packaged assets in production', async () => {
  const main = await readFile(`${projectRoot}/apps/desktop/src/main.ts`, 'utf8')
  assert.match(main, /const RENDERER_ORIGIN = app\.isPackaged \? APP_ORIGIN : WEB_ORIGIN/)
  assert.match(main, /const WS_RENDERER_ORIGIN = WEB_ORIGIN\.replace/)
  assert.match(main, /loadURL\(`\$\{RENDERER_ORIGIN\}\//)
  assert.match(main, /developmentConnectSource/)
  assert.match(main, /developmentScriptSource/)
})
